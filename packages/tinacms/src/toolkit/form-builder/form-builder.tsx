...
export const FormBuilder: FC<FormBuilderProps> = ({ form, onPristineChange, ...rest }) => {
  ...
  React.useEffect(() => {
    const isLikelyDocumentPath = (path: string | undefined): boolean => {
      if (!path) return false;
      // Crude check: allows paths with typical content extensions, rejects deep object paths like 'scope.content...'
      // and requires a dot, assuming file extensions.
      // Will allow 'content/pages/foo.md', 'data/global.json'
      // Will reject 'scope.content.field.0.title', 'myObjectField' (if no extension)
      const hasExtension = path.lastIndexOf('.') > path.lastIndexOf('/');
      return hasExtension && !path.startsWith('scope.') && !path.startsWith('_');
    };

    if (!isLikelyDocumentPath(tinaForm.relativePath)) {
      // If it's not a document path (e.g., a nested object form path),
      // collection-level beforeSubmit hooks don't apply.
      // Ensure it's reset if it was set previously and path changed.
      if (tinaForm.beforeSubmit !== undefined) {
        tinaForm.beforeSubmit = undefined;
      }
      return;
    }

    const collection = schema.getCollectionByFullPath(tinaForm.relativePath, true);
  ...
  }, [tinaForm, tinaForm.relativePath, schema, cms]);
