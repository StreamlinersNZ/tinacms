...
const FileItem = ({
  ...
  const form = React.useMemo(
    () =>
      node.formId
        ? cms.state.forms.find(({ tinaForm }) => tinaForm.id === node.formId)
        : undefined,
    [cms.state.forms, node.formId]
  );
  // Strip file extension for display
  const nameWithoutExtension = node.name.replace(/\.[^/.]+$/, '');
  const friendlyTitle =
    form?.tinaForm.values.title || form?.tinaForm.id || nameWithoutExtension;
  ...
  return (
    <div className='flex items-stretch'>
      ...
      >
        ...
        <div className='group-hover:text-blue-500 text-xs truncate leading-tight text-gray-500'>
          {friendlyTitle}
        </div>
      </button>
    </div>
